...
class SolveConflict:
    ...
    def __init__(self, data: data_source.DataSource):
        ...
        # 不同班级下课程对应的总课时数   马#['1班', '2班', '3班', '4班', '5班', '6班', '7班', '8班', '9班']
        ...
        # 手动排了某些课 [{'week': 2, 'section': 6, 'class': '1班', 'course': '体'},]

        self.static_part_schedule = copy.deepcopy(data.PART_SCHEDULE)

        ####################################增加变量#############################################
        self.static_linked_course_flag = data.LINKED_COURSE_FLAG

        self.class_rest_pos = data.CLASS_REST_POS

        # 1 ：初始化关联班级，link_classes = {“1班”：{“语”：[“2班”，“3班”，“4班”]}}，说明1234班有共同的语文老师
        # 2 ：教师某slot禁止，科目某slot禁止，fixed_not_assign= {11：{“teacher”：[“张三”，“李四”]，“course”：[“语”，“数”]}}
        # 3 ：初始化某slot固定不排课，fixed_not_assign={11：None}
        # 4 ：手动排课，fixed_assign={“1班11”：“语”}
        self.init_data()

    def init_data(self):
        # 初始化关联班级 class_course_teacher = ['1班': {'语': '毕姗', '数': '曹宇', '英': '路平',....,}，'2班'：{} , ...}]
        class_course_teacher = self.class_course_teacher
        for a_class in self.classes:
            self.link_classes[a_class] = {}

            # class_course_teacher[a_class]
            # ={'语': '毕姗', '数': '曹宇', '英': '路平', '物': '王碧雪', '化': '刘臣', '音-美': '王祎', '班': '刘臣', '体': '体-1', '政': '许鑫'}
        ...
        for teacher in self.static_not_assign_teacher:
            for time in self.static_not_assign_teacher[teacher]:
                ...
                if key in self.fixed_not_assign and self.fixed_not_assign[key] is not None:
                    ...
                    self.fixed_not_assign[key] = {'teacher': [teacher]}
        # 某个科目在某些时间点不排课，dict后不加东西默认遍历key
        for course in self.static_not_assign_course:
            for time in self.static_not_assign_course[course]:
                ...
                if key in self.fixed_not_assign and self.fixed_not_assign[key] is not None:
                    ...
                    self.fixed_not_assign[key] = {'course': [course]}
        # 初始化固定不排，dict后不加东西默认遍历key
        for time in self.static_not_assign_time:
            key = str(time['week'] - 1) + str(time['section'] - 1)
            # 假如本来该点就是要求的不排课，那么老师和科目的禁止自动成立，老师和科目的禁止就不算限制条件了
        ...
        if key in self.fixed_not_assign:
            ...
            if 'course' in condition:
                if course in condition['course']:
                    return True
            #     ########################################马原进行修改
            #    if 'teacher' in condition and course is not None:   张鑫原版代码
            if 'teacher' in condition and course in self.class_course_teacher[a_class]:
